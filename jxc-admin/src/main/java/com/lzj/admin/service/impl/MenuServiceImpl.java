...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements IMenuService {
    ...
    public List<TreeDto> queryAllMenus(Integer roleId) {
        List<TreeDto> treeDtos = this.baseMapper.queryAllMenus(roleId); //所有菜单的集合
        List<Integer> roleHasMenuIds= roleMenuService.queryRoleHasAllMenusByRoleId(roleId); //角色拥有的菜单的集合
        if(CollectionUtils.isNotEmpty(roleHasMenuIds)){
            treeDtos.forEach(treeDto -> { // 通过 拉姆达 做遍历
    ...
    public Map<String, Object> menuList() {
        List<Menu> menus = this.list(new QueryWrapper<Menu>().eq("is_del",0)); //调用父类的方法查询菜单数据
    ...
    public void saveMenu(Menu menu) {
        ...
        AssertUtil.isTrue(null !=this.findMenuByNameAndGrade(menu.getName(),menu.getGrade()),"该层级下菜单已存在!");  //不同层级菜单可以存在
